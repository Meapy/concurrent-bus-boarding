...
- The game options page provides an **Enable concurrent boarding** toggle, on by default. Turning it off hands every bus straight back to the game while keeping the overlays, the zone editor and the stop repair. It takes effect immediately, so a city can be compared with and without the feature in one session.
