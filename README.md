...
- Once a bus is admitted, it remains stopped at that exact queue position, even if space opens ahead. A follower uses the game's dwell, waiting-distance, and onboard-passenger readiness checks; only after all three finish does the mod run native end-boarding cleanup and advance it to the next waypoint.
