...
- The game options page provides an **Enable concurrent boarding** toggle, **off by default**. A bus held at a stop takes a little longer than the game's own boarding, and the game uses stop waiting times when residents choose a route, so on a busy network the feature can cost passengers. With it off, every bus is handed straight back to the game while the overlays, the zone editor and the stop repair keep working. It takes effect immediately, so a city can be compared with and without it in one session.
- Updating from a version before 1.5.1 switches the toggle off once, through a hidden `SettingsVersion` field that older settings files do not contain. The player's choice after that is remembered.
...
Thanks to **CheeseBunny_Gaming**, **Eiden3000** and **Minimumderp** for reporting the declining
bus ridership in 1.4.2 and sharing the detail that made it possible to track down and fix.
