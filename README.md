...
- Buses keep the game's native lane direction, collision avoidance, transforms, and current-lane occupancy. Before boarding starts, the mod may move only a real same-physical-lane `EndOfPath` navigation entry to its forward packed position. A strictly-ahead check prevents turning around; when that native endpoint is unavailable, the bus keeps its vanilla stop position and remains eligible to board.
