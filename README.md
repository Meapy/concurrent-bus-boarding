...
- Bus boarding zones are learned from approaching buses and shown as a translucent overlay on the road at the stop. The game options page provides the game's native colour wheel for the global overlay colour; the same wheel appears beside a selected stop when it uses the global colour. A bus within 40 m of its target waypoint makes its actual current lane authoritative; that physical observation remains cached over inferred route lanes. This keeps paired pull-in stops on their own side while preventing a distant junction or driveway from moving the zone.
...
- A separate confirmed **Reset all stop overlay colours** action returns every stop to the global colour without changing customized zone lengths.
- Select any served bus stop to open **Bus Boarding Zone** below the transit-lines section. Before a bus arrives, the initial zone is inferred from that stop's connected route waypoint; a nearby bus later confirms the exact physical driven lane. The **Length** slider sizes the rearward zone from 6 m to 200 m. **Use line colour** switches that stop from the global colour to the first served line shown in the native Lines list; its RGB follows later line-colour changes while transparency remains global. **Edit on map** focuses the camera on the stop and adds cyan handles at the rear edge; the stop-side edge remains fixed. The rear edge follows the line's native inbound lane path across ordinary road-segment and junction boundaries instead of stopping at the first lane curve. Right-click or press Escape to leave map editing. The edited length and colour source are saved on that stop with the city; **Use automatic** removes the length override and **Use global colour** removes only the colour override. Existing saves retain their serialized offset field for compatibility, but it is intentionally ignored.
